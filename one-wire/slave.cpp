...
void loop()
{
    ...
    if (messageReceived)
    {
        AES_CBC_decrypt_buffer(&ctx, receivedData, BUF_LEN);
        ...
        for (int i = 0; i < BUF_LEN; i++)
            if (receivedData[i] >= 32 && receivedData[i] <= 126)
                Serial.print((char)receivedData[i]); // Print as character
        Serial.println(")");

        AES_ctx_set_iv(&ctx, iv);
        memset(receivedData, 0, BUF_LEN);
